	...
	b.fn.fullpage = function(c) {
		...
		function O() {
			...
			if (!c.autoScrolling && !f.hasClass("active")) {
				D = !0;
				var E = b(".fp-section.active").index(".fp-section") + 1,
					g = F(f),
					r = f.data("anchor");
				f.addClass("active").siblings().removeClass("active");
				h || (b.isFunction(c.onLeave) && c.onLeave.call(this, E, f.index(".fp-section") + 1, g), b.isFunction(c.afterLoad) && c.afterLoad.call(this, r, f.index(".fp-section") + 1));
				G(r, 0);
				c.anchors.length && !h && (p = r, location.hash = r);
				clearTimeout(P);
				P = setTimeout(function() {
					D = !1
				}, 100)
			}
			c.scrollBar && (clearTimeout(Q), Q = setTimeout(function() {
				h || m(f)
			}, 1E3))
		}
		function R(a) {
		...
		function ma(a) {
			...
			if (!S(a.target)) {
				...
				var e = R(a);
				h || t || (d = T(d), q = d.y, z = d.x, a.find(".fp-slides").length && Math.abs(A - z) > Math.abs(s - q) ? Math.abs(A - z) > b(window).width() / 100 * c.touchSensitivity && (A > z ? b.fn.fullpage.moveSlideRight() : b.fn.fullpage.moveSlideLeft()) : c.autoScrolling && !c.scrollBar && Math.abs(s - q) > b(window).height() / 100 * c.touchSensitivity && (s > q ? y("down", e) : q > s && y("up", e)))
			}
		}
		function S(a, d) {
			d = d || 0;
			var e = b(a).parent();
			return d < c.normalScrollElementTouchThreshold && e.is(c.normalScrollElements) ? !0 : d == c.normalScrollElementTouchThreshold ? !1 : S(e, ++d)
		}
		function na(a) {
			a = T(a.originalEvent);
			s = a.y;
		...
		function m(a, d, e) {
			...
			if ("undefined" !== typeof f && (d = {
				...
				if (d.activeSlide.length) var E = d.activeSlide.data("anchor"),
					g = d.activeSlide.index();
				c.autoScrolling && c.continuousVertical && "undefined" !== typeof d.isMovementUp && (!d.isMovementUp && "up" == d.yMovement || d.isMovementUp && "down" == d.yMovement) && (d.isMovementUp ? b(".fp-section.active").before(d.activeSection.nextAll(".fp-section")) : b(".fp-section.active").after(d.activeSection.prevAll(".fp-section").get().reverse()), k(b(".fp-section.active").position().top), V(), d.wrapAroundElements = d.activeSection, d.dest = d.element.position(), d.dtop = d.dest.top, d.yMovement = F(d.element));
				a.addClass("active").siblings().removeClass("active");
				h = !0;
				W(g, E, d.anchorLink, d.sectionIndex);
				b.isFunction(c.onLeave) && !d.localIsResizing && c.onLeave.call(this, d.leavingSection, d.sectionIndex + 1, d.yMovement);
				oa(d);
				p = d.anchorLink;
				c.autoScrolling && G(d.anchorLink, d.sectionIndex)
			}
		}
		function oa(a) {
			if (c.css3 && c.autoScrolling && !c.scrollBar) X("translate3d(0px, -" + a.dtop + "px, 0px)", !0), setTimeout(function() {
				Y(a)
			}, c.scrollingSpeed);
			else {
				var d = pa(a);
		...
		function pa(a) {
			...
			c.autoScrolling && !c.scrollBar ? (b.options = {
				top: -a.dtop
			}, b.element = "." + Z) : (b.options = {
				scrollTop: a.dtop
			}, b.element = "html, body");
			return b
		}
		function qa(a) {
			a.wrapAroundElements && a.wrapAroundElements.length && (a.isMovementUp ? b(".fp-section:first").before(a.wrapAroundElements) : b(".fp-section:last").after(a.wrapAroundElements), k(b(".fp-section.active").position().top), V())
		}
		function Y(a) {
			qa(a);
		...
		function u(a, d) {
			...
			if (c.onSlideLeave) {
				...
				p || "none" === q || b.isFunction(c.onSlideLeave) && c.onSlideLeave.call(this, l, r + 1, n, q)
			}
			...
			h.hasClass("active") && W(g, k, l, r);
			var s = function() {
					p || b.isFunction(c.afterSlideLoad) && c.afterSlideLoad.call(this, l, r + 1, k, g);
					t = !1
				};
			c.css3 ? (e = "translate3d(-" + e.left + "px, 0px, 0px)", aa(a.find(".fp-slidesContainer"), 0 < c.scrollingSpeed).css(ba(e)), setTimeout(function() {
				s()
		...
		function ca() {
			...
			if (K) {
				if ("text" !== b(document.activeElement).attr("type")) {
					...
					Math.abs(a - L) > 20 * Math.max(L, a) / 100 && (b.fn.fullpage.reBuild(!0), L = a)
				}
			} else clearTimeout(ea), ea = setTimeout(function() {
		...
		function aa(a, b) {
			...
			return b ? (a.removeClass("fp-notransition"), a.css({
				"-webkit-transition": e,
				transition: e
			})) : M(a)
		}
		...
		function ra(a, d) {
			if (825 > a || 900 > d) {
				...
				if(b("body").hasClass('fp-viewing-page6')){
					var _size=b("body").css("font-size");
					if("undefined" != typeof _size && _size.indexOf("%")!=-1){
						b("body").css("font-size",_size);
					}else{
						b("body").css("font-size","");
					}
				}else{
		...
		function x(a) {
			...
			b = w - parseInt(b.css("padding-bottom")) - parseInt(b.css("padding-top"));
		...
		function ga(a) {
			a.addClass("fp-table").wrapInner('<div class="fp-tableCell" style="height:' + ha(a) + 'px;" />')
		}
		function ha(a) {
			var b = w;
			if (c.paddingTop || c.paddingBottom) b = a, b.hasClass("fp-section") || (b = a.closest(".fp-section")), a = parseInt(b.css("padding-top")) + parseInt(b.css("padding-bottom")), b = w - a;
			return b
		}
		function X(a, b) {
			aa(g, b);
		...
		function J(a, c) {
			...
			a === p || e.hasClass("active") ? ia(e, c) : m(e, function() {
				ia(e, c)
			})
		}
		function ia(a, b) {
		...
		function W(a, b, e, f) {
			var g = "";
			c.anchors.length ? (a ? ("undefined" !== typeof e && (g = e), "undefined" === typeof b && (b = a), I = b, location.hash = g + "/" + b) : ("undefined" !== typeof a && (I = b), location.hash = e), B(location.hash)) : "undefined" !== typeof a ? B(f + "-" + a) : B(String(f))
		}
		function B(a) {
			a = a.replace("/", "-").replace("#", "");
			b("body")[0].className = b("body")[0].className.replace(/\b\s?fp-viewing-.*\b/g, "");
		...
		function H(a) {
			...
			u(a.closest(".fp-slides"), a);
			b.fn.fullpage.setScrollingSpeed(d)
		}

		function k(a) {
			c.scrollBar ? g.scrollTop(a) : c.css3 ? X("translate3d(0px, -" + a + "px, 0px)", !1) : g.css("top", -a)
		}
		function ba(a) {
			return {
				"-webkit-transform": a,
		...
		b.fn.fullpage.setAutoScrolling = function(a) {
			...
			c.autoScrolling && !c.scrollBar ? (b("html, body").css({
				...
				"touch-action": "none"
			}), a.length && k(a.position().top)) : (b("html, body").css({
				overflow: "visible",
				height: "initial"
			}), g.css({
				"-ms-touch-action": "",
				"touch-action": ""
			}), k(0), b("html, body").scrollTop(a.position().top))
		};
		...
		b.fn.fullpage.setAllowScrolling = function(a) {
			if (a) {
				if (b.fn.fullpage.setMouseWheelScrolling(!0), K || ka) MSPointer = ja(), b(document).off("touchstart " + MSPointer.down).on("touchstart " + MSPointer.down, na), b(document).off("touchmove " + MSPointer.move).on("touchmove " + MSPointer.move, ma)
			} else if (b.fn.fullpage.setMouseWheelScrolling(!1), K || ka) MSPointer = ja(), b(document).off("touchstart " + MSPointer.down), b(document).off("touchmove " + MSPointer.move)
		};
		...
		b.fn.fullpage.moveTo = function(a, c) {
			...
			"undefined" !== typeof c ? J(a, c) : 0 < e.length && m(e)
		};
		...
		b.fn.fullpage.reBuild = function(a) {
			...
			b(".fp-section").each(function() {
				...
				c.verticalCentered && b(this).find(".fp-tableCell").css("height", ha(b(this)) + "px");
				b(this).css("height", e + "px");
				if (c.scrollOverflow) {
					var a = b(this).find(".fp-slide");
					a.length ? a.each(function() {
						x(b(this))
				...
				a.length && u(a, a.find(".fp-slide.active"))
			});
		...
		var t = !1,
			...
			h = !1,
			v = !1,
			p, I, l, Z = "fullpage-wrapper";
		b.fn.fullpage.setAllowScrolling(!0);
		c.css3 && (c.css3 = ta());
		b(this).length ? (g.css({
			height: "100%",
			position: "relative"
		}), g.addClass(Z)) : C("error", "Error! Fullpage.js needs to be initialized with a selector. For example: $('#myContainer').fullpage();");
		...
		b(".fp-section").each(function(a) {
			...
			if (1 < f) {
				...
				b(this).find(".fp-slides").after('<div class="fp-controlArrow fp-prev"></div><div class="fp-controlArrow fp-next"></div>');
				"#fff" != c.controlArrowColor && (b(this).find(".fp-controlArrow.fp-next").css("border-color", "transparent transparent transparent " + c.controlArrowColor), b(this).find(".fp-controlArrow.fp-prev").css("border-color", "transparent " + c.controlArrowColor + " transparent transparent"));
				c.loopHorizontal || b(this).find(".fp-controlArrow.fp-prev").hide();
				c.slidesNavigation && sa(b(this), f);
				e.each(function(a) {
					b(this).css("width", g + "%");
					c.verticalCentered && ga(b(this))
				});
				d = d.find(".fp-slide.active");
				0 == d.length ? e.eq(0).addClass("active") : H(d)
			} else c.verticalCentered && ga(b(this))
		}).promise().done(function() {
			b.fn.fullpage.setAutoScrolling(c.autoScrolling);
			var a = b(".fp-section.active").find(".fp-slide.active");
			a.length && (0 != b(".fp-section.active").index(".fp-section") || 0 == b(".fp-section.active").index(".fp-section") && 0 != a.index()) && H(a);
			c.fixedElements && c.css3 && b(c.fixedElements).appendTo("body");
			c.navigation && (l.css("margin-top", "-" + l.height() / 2 + "px"), l.find("li").eq(b(".fp-section.active").index(".fp-section")).find("a").addClass("active"));
			c.menu && c.css3 && b(c.menu).closest(".fullpage-wrapper").length && b(c.menu).appendTo("body");
			c.scrollOverflow ? ("complete" === document.readyState && N(), b(window).on("load", N)) : b.isFunction(c.afterRender) && c.afterRender.call(this);
			da();
			a = window.location.hash.replace("#", "").split("/")[0];
			if (a.length) {
				var d = b('[data-anchor="' + a + '"]');
				!c.animateAnchor && d.length && (c.autoScrolling ? k(d.position().top) : (k(0), B(a), b("html, body").scrollTop(d.position().top)), G(a, null), b.isFunction(c.afterLoad) && c.afterLoad.call(this, a, d.index(".fp-section") + 1), d.addClass("active").siblings().removeClass("active"))
			}
		...
		b(document).keydown(function(a) {
			if (c.keyboardScrolling && c.autoScrolling && (40 != a.which && 38 != a.which || a.preventDefault(), !h)) switch (a.which) {
			case 38:
			case 33:
		...
		b.fn.fullpage.destroy = function(a) {
			...
			b(window).off("scroll", O).off("hashchange", $).off("resize", ca);
